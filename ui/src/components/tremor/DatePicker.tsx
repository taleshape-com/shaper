...
const triggerStyles = tv({
  base: [
    // base
    "peer flex w-full cursor-pointer appearance-none items-center gap-x-2 truncate rounded-md border px-2 py-1.5 outline-none transition-all text-sm",
    // background color
    "bg-cbg dark:bg-dbg",
    // border color
    "border-cb dark:border-db",
...
const Trigger = React.forwardRef<HTMLButtonElement, TriggerProps>(
  (
    { className, children, placeholder, hasError, isDefaultValue, ...props }: TriggerProps,
    forwardedRef,
  ) => {
    return (
  ...
  return (
    <PopoverPrimitives.Portal container={container}>
      <PopoverPrimitives.Content
        ...
        className={cx(
          // base
          "relative z-50 w-fit rounded-sm border text-sm shadow-xl shadow-black/[2.5%] select-none",
          // widths
...
const RangeDatePicker = ({
  ...
  return (
    ...
    >
      ...
      <CalendarPopover align={align}>
        <div className="flex bg-cbgs dark:bg-dbgs">
