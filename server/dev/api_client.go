...
func (c *APIClient) refreshToken(ctx context.Context) error {
	...
	if resp.StatusCode == http.StatusUnauthorized {
		resp.Body.Close()
		return fmt.Errorf("authentication failed (401 Unauthorized): please run 'shaper login'")
	}
...
func (c *APIClient) authedRequest(ctx context.Context, method, path string, body []byte) (*http.Response, error) {
	...
	if resp.StatusCode == http.StatusUnauthorized {
		...
		return nil, fmt.Errorf("request failed with status 401 Unauthorized: please run 'shaper login'")
	}
	return resp, nil
}
